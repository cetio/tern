...
public:
    ...
    @trusted string disassemble(ubyte[] bytes)
    {
        ...
        string prefix;
        bool keepPrefix;
        BinaryStream bs = new BinaryStream(bytes);

        while (bs.mayRead)
        {
            if (!instructions.keys.filter!(key => instructions[key] == bs.peek!ubyte(1)).empty)
            {
                string mnemonic = instructions.keys.filter!(key => instructions[key] == bs.peek!ubyte(1)).front;
                bs.step!ubyte(1);
                ...
                switch (mnemonic)
                {
                    ...
                    case "brtrue":
                    case "brzero":
                        disassembly ~= format("%08d: %s%s $loc_%X\n", bs.position - 1, prefix, mnemonic, bs.read!int);
                        break;
                    case "beq.s":
                    case "bge.s":
                    case "bge.un.s":
                    ...
                    case "brtrue.s":
                    case "brzero.s":
                        disassembly ~= format("%08d: %s%s $loc_%X\n", bs.position - 1, prefix, mnemonic, bs.read!byte);
                        break;
                    case "calli":
                        disassembly ~= format("%08d: %s%s %08d\n", bs.position - 1, prefix, mnemonic, bs.read!ptrdiff_t);
                        break;
                    case "box":
                    case "call":
                    case "callvirt":
                    ...
                    case "unbox":
                    case "unbox.any":
                        disassembly ~= format("%08d: %s%s %X\n", bs.position - 1, prefix, mnemonic, bs.read!uint);
                        break;
                    case "ldarg.s":
                    case "ldarga.s":
                    case "ldloc.s":
                    ...
                    case "starg.s":
                    case "stloc.s":
                        disassembly ~= format("%08d: %s%s %X\n", bs.position - 1, prefix, mnemonic, bs.read!ubyte);
                        break;
                    case "ldc.i4.s":
                    case "leave.s":
                        disassembly ~= format("%08d: %s%s %X\n", bs.position - 1, prefix, mnemonic, bs.read!byte);
                    ...
                    case "leave":
                        ...
                        break;
                    case "ldc.i8":
                        disassembly ~= format("%08d: %s%s %X\n", bs.position - 1, mnemonic, bs.read!long);
                        break;
                    case "ldc.r4":
                        disassembly ~= format("%08d: %s%s %f\n", bs.position - 1, prefix, mnemonic, bs.read!float);
                        break;
                    case "ldc.r8":
                        disassembly ~= format("%08d: %s%s %f\n", bs.position - 1, prefix, mnemonic, bs.read!double);
                        break;
                    case "switch":
                        disassembly ~= format("%08d: %s%s %s\n", bs.position - 1, prefix, mnemonic, bs.read!int(bs.read!uint).to!string);
                        break;
                    default:
                        disassembly ~= format("%08d: %s%s\n", bs.position - 1, prefix, mnemonic);
                        break;
                }
            }
            else if (bs.mayRead(2) < bytes.length &&
                    !instructions.keys.filter!(key => instructions[key] == bs.peek!ubyte(2)).empty)
            {
                string mnemonic = instructions.keys.filter!(key => instructions[key] == bs.peek!ubyte(2)).front;
                bs.step!ubyte(2);
                ...
                switch (mnemonic)
                {
                    ...
                    case "starg":
                    case "stloc":
                        disassembly ~= format("%08d: %s%s %X\n", bs.position - 1, prefix, mnemonic, bs.read!ushort);
                        break;
                    case "no":
                        prefix ~= "no.";
                        keepPrefix = true;
            ...
            {
                if (bs.mayRead(2))
                    throw new Exception(format("Invalid bytes for an IL instruction (0..1): %X", bs.read!ubyte));

                throw new Exception(format("Invalid bytes for an IL instruction (0..2): %(%02X%)", bs.read!ubyte(2)));
            }
