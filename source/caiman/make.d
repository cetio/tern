...
public template make(alias root, string dest)
{
    ...
    {
        const string rootName = fullyQualifiedName!root.toPascalCase();
        const string cs = dest~"\\csharp\\"~fullyQualifiedName!root;
        foreach (mod; getImports!root)
        {
            ...
            foreach (type; __traits(allMembers, mod)[2..$])
            {
                ...
                static if (isType!T && !__traits(isTemplate, T) && isAggregateType!T)
                {
                    ...
                    static foreach (i; 0..getImplements!T.length)
                        tinherits ~= getImplements!T[i].stringof;
                    ...
                    foreach (member; __traits(allMembers, T))
                    {
                        ...
                        else static if (isCallable!A && isFunction!A && isExport!A)
                        {
                            ...
                            retType = retType.replace(modName, rootName);
                            ...
                            static if (isStaticArray!(ReturnType!A))
                            {
                                ...
                                file.append("    public "~retType~" "~__traits(identifier, A).toPascalCase()~" { get { return "~mangle~"_get(_ptr); } set { "~mangle~"_set(_ptr, value) } }\n");
                            }
                            else static if (isPointer!(ReturnType!A))
                                file.append("    public "~retType~" "~__traits(identifier, A).toPascalCase()~" { get { return ("~retType~")Core.Importer.Call<nint>(\""~mangle~"_get\", _ptr); } set { Core.Importer.Call<nint>(\""~mangle~"_get\", _ptr, (nint)value); } }\n");
                            else
                                file.append("    public "~retType~" "~__traits(identifier, A).toPascalCase()~" { get { return Core.Importer.Call<"~retType~">(\""~mangle~"_get\", _ptr); } set { Core.Importer.Call<"~retType~">(\""~mangle~"_get\", _ptr, value); } }\n");
                        }
                    }
                }
            }
